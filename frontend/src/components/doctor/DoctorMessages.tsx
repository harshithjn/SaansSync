...
export default function DoctorMessages() {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 h-[calc(100vh-220px)] font-['Matter_Regular',sans-serif]">
                                    ...
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold transition-colors ${
                                                selectedPatientId === thread.patient_id ? 'bg-white/10' : 'bg-slate-100 text-slate-400'
                                            }`}>
                                                {thread.patient?.full_name?.charAt(0) || 'P'}
                                            </div>
                                            <span className="text-sm font-bold tracking-tight text-slate-950">{thread.patient?.full_name || 'Patient'}</span>
                                        </div>
                                        <span className={`text-[8px] font-bold uppercase tracking-widest ${
                                            selectedPatientId === thread.patient_id ? 'text-slate-400' : 'text-slate-300'
                                        }`}>
                                            {formatDate(thread.created_at)}
                                        </span>
                                    </div>
                                    <p className={`text-[11px] font-medium truncate ${
                                        selectedPatientId === thread.patient_id ? 'text-slate-400' : 'text-slate-500'
                                    }`}>{thread.content}</p>
                        ...
                        <div className="px-10 py-6 border-b border-slate-50 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-10 transition-all duration-500">
                            <div className="flex items-center gap-5">
                                <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center border border-slate-100 group">
                                     <img 
                                        src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${threads.find(t => t.patient_id === selectedPatientId)?.patient?.full_name}`} 
                                        alt="Avatar" 
                        ...
                        <div ref={scrollRef} className="flex-1 p-10 overflow-y-auto no-scrollbar bg-slate-50/30">
                            <div className="space-y-8 max-w-4xl mx-auto">
                                ...
                                {activeConversation.map(msg => (
                                    <div key={msg.id} className={`flex ${msg.sender_role === 'doctor' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-500`}>
                                        <div className={`group flex flex-col ${msg.sender_role === 'doctor' ? 'items-end' : 'items-start'}`}>
                                            ...
                                            <div className="flex items-center gap-2 mt-3 px-2">
                                                ...
                                                {msg.sender_role === 'doctor' && <ShieldCheck className="w-2.5 h-2.5 text-emerald-400" />}
                                            </div>
                                        </div>
                                    </div>
                                ))}
